...
@DisplayName("키워드 답장 편지 JPA 엔티티 테스트")
class KeywordReplyComplaintEntityTest {
    ...
    void from() {
        ...
        // when
        KeywordReplyComplaintEntity jpaEntity = KeywordReplyComplaintEntity.from(complaint);
    ...
    void toDomain() {
        // given
        LocalDateTime now = LocalDateTime.now();
        KeywordReplyComplaintEntity jpaEntity = KeywordReplyComplaintEntity.builder()
                .id(1L)
                .letterId(1L)
                .reporterId(1L)
                .description("욕설 사용")
