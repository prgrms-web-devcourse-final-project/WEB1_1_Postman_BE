...
@DisplayName("지도 편지 JPA 엔티티 테스트")
class MapComplaintEntityTest {
    ...
    void from() {
        ...
        // when
        MapComplaintEntity jpaEntity = MapComplaintEntity.from(complaint);
    ...
    void toDomain() {
        // given
        LocalDateTime now = LocalDateTime.now();
        MapComplaintEntity jpaEntity = MapComplaintEntity.builder()
                .id(1L)
                .letterId(1L)
                .reporterId(1L)
                .description("욕설 사용")
