...
class MapLetterJpaRepositoryTest {
    ...
    @Test
    @DisplayName("createUserId가 특정 userId이고 삭제되지 않고, 블락되지 않은 편지 조회에 성공하며 페이징이 올바르게 동작한다.")
    void findActiveByCreateUserIdWithPaging() {
        // given
        Long userId = 1L;
        int page = 0; // 페이지 번호 (0부터 시작)
        int size = 3; // 페이지 크기 (한 페이지에 포함될 데이터 수)
        Pageable pageable = PageRequest.of(page, size);

        // when
        Page<MapLetterEntity> resultPage = mapLetterJpaRepository.findActiveByCreateUserId(userId, pageable);

        // then
        assertNotNull(resultPage); // 반환된 페이지 객체가 null이 아닌지 확인
        assertEquals(page, resultPage.getNumber()); // 페이지 번호가 요청한 값과 일치하는지 확인
        assertEquals(size, resultPage.getSize()); // 페이지 크기가 요청한 값과 일치하는지 확인
        assertTrue(resultPage.getContent().size() <= size); // 반환된 데이터 수가 페이지 크기 이하인지 확인
        assertTrue(resultPage.getContent().stream().noneMatch(MapLetterEntity::isDeleted)); // 삭제되지 않은 편지만 포함
        assertTrue(resultPage.getContent().stream().noneMatch(MapLetterEntity::isBlocked)); // 블락되지 않은 편지만 포함
        assertTrue(resultPage.getContent().stream().allMatch(letter -> letter.getCreateUserId().equals(userId))); // 특정 userId의 편지만 포함
    }
    ...
    void findActiveByTargetUserId() {
        ...
        // when
        Page<MapLetterEntity> result = mapLetterJpaRepository.findActiveByTargetUserId(targetUserId, pageable);

        // then
        assertNotNull(result);
        assertTrue(result.getContent().stream().noneMatch(MapLetterEntity::isDeleted));
        assertTrue(result.getContent().stream().noneMatch(MapLetterEntity::isBlocked));
